...
foreach ($sub in $subs)
{
    ...
    #{
        ...
        Write-Output "Listing Linus Virtal Machines in subscription '$($sub.Name)'"
        $VMs = Get-AzVM -Status | Where-Object {$_.StorageProfile.OsDisk.OsType.ToString() -eq "Linux"}
        $VMsSorted = $VMs | Sort-Object -Property ResourceGroupName
        Write-Output "Total number of VMs in this subscription" $VMsSorted
        $PreviousRG = ""
        foreach($VM in $VMs)
        {
            ...
            $AutoUpgradeMinorVersion = Get-AzVMExtension -VMName $vm.Name -ResourceGroupName $vm.ResourceGroupName | where Publisher -eq "Microsoft.EnterpriseCloud.Monitoring" | Select-Object -ExpandProperty AutoUpgradeMinorVersion
            ...
            if ($check.Value.Message.Contains("/opt/omi/bin/omiserver:"))
            {
                ...
                if($pkgVer -eq "OMI-1.6.8-1")
                {
                    ...
                    if ($AutoUpgradeMinorVersion) {Write-Host -ForegroundColor Green "`t`t`t AutoUpgradeMinorVersion: true"}
                    else {Write-Host -ForegroundColor Red "`t`t`t AutoUpgradeMinorVersion: false"}
                }
                else
                {
